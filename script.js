...
const R_HUB_OUT     = R_OUT * 0.45;
const R_HUB_IN      = R_OUT * 0.18;
...
let ballAngle = -Math.PI/2, ballSpeed = 0;
...
function colorFor(n){ if(n===0) return "#1aa34a"; return REDS.has(n) ? "#cc2a2a" : "#0d0d0d"; }
...
function draw(){
  ...
  ctx.arc(CX, CY, R_POCKETS_IN, 0, TAU, true); ctx.closePath(); ctx.fill();
  ...
  ctx.arc(0,0,R_HUB_IN,0,TAU,true); ctx.fill();
  ...
  ballGrad.addColorStop(0,"#fff7dd"); ballGrad.addColorStop(1,"#bba46b");
  ctx.fillStyle = ballGrad; ctx.beginPath(); ctx.arc(x,y,R_BALL,0,TAU); ctx.fill();
  ctx.restore();
}
...
function tick(now){
  ...
  if(spinning){
    wheelAngle = mod(wheelAngle + wheelSpeed*dt, TAU); wheelSpeed *= 0.995;
    ballAngle  = mod(ballAngle  - ballSpeed *dt, TAU); ballSpeed  *= 0.992;

    if(!ballFalling && wheelSpeed < 0.8) ballFalling = true;
    if(ballFalling){
      ballRadius -= ( (R_POCKETS_OUT + R_POCKETS_IN)/2 - (R_HUB_OUT*0.75) ) * dt * 0.5;
      if(ballRadius < (R_POCKETS_IN + R_HUB_OUT)/2) ballRadius = (R_POCKETS_IN + R_HUB_OUT)/2;
    }
    if(wheelSpeed < 0.15 && ballSpeed < 0.15){ spinning = false; snapToWinning(); }
  }
...
function startSpin(){
  if(spinning) return;
  wheelSpeed = 6 + Math.random()*2;
  ballSpeed  = 11 + Math.random()*3;
  ballRadius = (R_POCKETS_OUT + R_POCKETS_IN)/2;
  ballFalling = false; spinning = true;
  ui.winNum.textContent = "…"; ui.winColor.textContent = "…";
}
function reset(){
  spinning=false; wheelAngle=0; wheelSpeed=0; ballAngle=-Math.PI/2; ballSpeed=0;
  ballRadius=(R_POCKETS_OUT + R_POCKETS_IN)/2; ballFalling=false;
...
function snapToWinning(){
  ...
  ballAngle = centerAngle; ballFalling = true; ballRadius = (R_POCKETS_IN + R_HUB_OUT)/2;
